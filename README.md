...
Add a stable root `id` such as `product-growth`. Synced dashboards appear in the authenticated dashboard workspace and its title-adjacent switcher; the switcher includes every project-visible dashboard and your own personal dashboards. Sync does not create a standalone or public URL. Personal `aat_` tokens need `dashboards` scope and default new dashboards to personal visibility. `--visibility project` shares the dashboard with current project members; use it with a directory sync when teammates should see every file. Later syncs preserve visibility unless the flag is explicit. Identical validated content keeps the current server version. Projects without a visible dashboard show the starter, preview, and sync workflow in the web workspace.
