...
Add a stable root `id` such as `product-growth`. Synced dashboards appear in the authenticated dashboard workspace and its title-adjacent switcher; sync does not create a standalone or public URL. Personal `aat_` tokens need `dashboards` scope and default new dashboards to personal visibility. `--visibility project` shares the dashboard with current project members. Later syncs preserve visibility unless the flag is explicit. Identical validated content keeps the current server version.
